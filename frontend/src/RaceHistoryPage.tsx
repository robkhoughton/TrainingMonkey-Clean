import React, { useState, useEffect } from 'react';
import styles from './TrainingLoadDashboard.module.css';
import { usePagePerformanceMonitoring, useComponentPerformanceMonitoring } from './usePerformanceMonitoring';
import RaceHistoryManager from './RaceHistoryManager';

interface RaceHistory {
  id: number;
  race_date: string;
  race_name: string;
  distance_miles: number;
  finish_time_minutes: number;
}

const RaceHistoryPage: React.FC = () => {
  // Performance monitoring
  usePagePerformanceMonitoring('race-history');
  const perfMonitor = useComponentPerformanceMonitoring('RaceHistoryPage');

  const [raceHistory, setRaceHistory] = useState<RaceHistory[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);

  const fetchRaceHistory = async () => {
    try {
      perfMonitor.trackFetchStart();
      const response = await fetch('/api/coach/race-history');
      if (!response.ok) {
        throw new Error('Failed to fetch race history');
      }
      const data = await response.json();
      setRaceHistory(data.history || []);
      perfMonitor.trackFetchEnd();
      perfMonitor.reportMetrics(data.history?.length || 0);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to load race history';
      setError(errorMessage);
      perfMonitor.reportMetrics(0, errorMessage);
    } finally {
      setIsLoading(false);
    }
  };

  useEffect(() => {
    fetchRaceHistory();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleHistoryChange = () => {
    fetchRaceHistory();
  };

  if (isLoading) {
    return (
      <div className={styles.dashboardContainer}>
        <div className={styles.card} style={{ textAlign: 'center', padding: '60px 20px' }}>
          <div className={styles.spinner}></div>
          <p style={{ marginTop: '20px', color: '#7f8c8d' }}>
            Loading race history...
          </p>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className={styles.dashboardContainer}>
        <div className={styles.card} style={{ textAlign: 'center', padding: '60px 20px' }}>
          <h2 style={{ color: '#e74c3c', marginBottom: '20px' }}>⚠️ Error Loading Race History</h2>
          <p style={{ color: '#7f8c8d', marginBottom: '30px' }}>{error}</p>
          <button
            onClick={fetchRaceHistory}
            style={{
              padding: '12px 24px',
              fontSize: '16px',
              backgroundColor: '#3498db',
              color: 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: 'pointer'
            }}
          >
            Retry
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className={styles.dashboardContainer}>
      <div className={styles.card} style={{ marginBottom: '20px' }}>
        <RaceHistoryManager history={raceHistory} onHistoryChange={handleHistoryChange} />
      </div>
    </div>
  );
};

export default RaceHistoryPage;


