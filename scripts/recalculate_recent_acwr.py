#!/usr/bin/env python3
"""
Recalculate ACWR for recent dates only (last 60 days)
"""
...
def recalculate_recent_acwr(user_id, days_back=60):
    """Recalculate ACWR for recent dates only"""

    print(f"\n{'='*80}")
    print(f"Recalculating ACWR for User {user_id} (Last {days_back} Days)")
    print(f"{'='*80}\n")
    ...
    # Get all unique dates with activities in range
    ...
    if dates:
        ...
        if latest:
            data = latest[0]
            print(f"\nNew ACWR Values (with decay rate = 0.02):")
            ...
            # Calculate what it should be with 0.02 decay
            print(f"\n  (Compare to old: External ACWR was 1.630 with decay=0.05)")

    print(f"\n{'='*80}\n")

if __name__ == '__main__':
    recalculate_recent_acwr(user_id=1, days_back=60)
