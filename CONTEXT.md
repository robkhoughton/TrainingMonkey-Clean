...
**Daily Recommendation** (`llm_recommendations` table): `daily_recommendation` (prose) + `structured_output` (JSONB: assessment, divergence, risk, context). **Three** generators exist, not two: `generate_recommendations()` (standard, nightly cron for all users), `generate_autopsy_informed_daily_decision()` (journal-triggered, post-workout — the most frequently hit path) and `generate_recommendations_agentic()` (tool-calling, feature-flag gated).

**Context Seam** (`assemble_daily_context()` → `DailyContext`): the single place a daily prompt's context blocks are built — the input-side mirror of `finalize_recommendation()`. The standard and autopsy-informed builders both assemble through it and are now at full signal parity; each keeps its own prompt template and wording, so this guarantees parity of *data*, not of voice. Adding a coaching signal here reaches those paths by construction; adding it to one builder's f-string is how they drifted before (readiness/HRV, dynamic AeT and the coaching-context library reached some paths but not others, while free-text journal notes reached only the post-workout path). `app/tests/test_daily_context.py` fails the build if a builder stops consuming a shared signal.
All three generators are now on the seam (agentic migrated 2026-08-24). The agentic path is a deliberate partial: it is two-turn, so it pre-injects only blocks no tool can supply (metric verdict, effective AeT, readiness, training stage, recent execution, preference feedback, autopsy) and leaves anything `TOOL_DEFINITIONS` exposes — activities, race goals, weekly program day, journal entries, athlete model — for the model to request. Those exemptions are declared in `test_daily_context.py::AGENTIC_TOOL_DELEGATED` and guarded by a test asserting the backing tools still exist.

_Fixed 2026-08-24, was live_: `call_claude_with_tools()` had no `system` parameter and never passed one to the SDK, so the agentic path assembled a full system prompt (weekly plan rule, deviation rules, tone, readiness, weekly strategic context, training guide, coaching-context library) and discarded it — only the user turn reached the model. Every earlier parity fix to that path was dead code. Always pass `system=`; the function now warns if you don't.
