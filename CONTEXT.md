...
_Remaining gap_: `generate_recommendations_agentic()` still assembles its prompt inline, including a hand-copied duplicate of the readiness block, and never receives the metric verdict, athlete model, race context or recent-execution blocks. It is tracked in that test file's `BUILDERS_NOT_YET_ON_SEAM`.
