...
**ACWR Sweet Spot** — status as of 2026-08-03: **retired at the code level, still an open product/branding question.**
...
- `acwr_sweet_spot_confidence` — a per-autopsy counter that fed a `model_calibrated: true/false` badge on the Race Readiness card's ACWR ceiling tile — was removed 2026-08-03. It was diagnosed as meaningless by the project's own QC audit ("confidence in an undefined sweet spot") and there was no honest signal to replace it with, since the ceiling it claimed to describe is never personalized. Displaying "calibrated" would have been false regardless of which counter drove it, so the badge was removed rather than re-pointed at a different stat.
- Still marketed in the brand framework doc as a live UI concept ("Sweet Spot," Success Green) and listed in `COACHING_FRAMEWORK_GAPS.md` as a planned user-facing feature (surfacing sweet spot low/high + confidence%) — **this part is unresolved**, see Open Questions.
...
3. **ACWR Sweet Spot** — PARTIALLY RESOLVED 2026-08-03. The dead/misleading code-level plumbing is gone (see above — `acwr_sweet_spot_confidence` removed, the stale `model_calibrated` badge removed from the Race Readiness card). What's still genuinely open, and is Rob's call, not an engineering one: the brand framework doc and `COACHING_FRAMEWORK_GAPS.md` both still describe "ACWR Sweet Spot" as a concept worth surfacing to users — either (a) build real per-athlete ACWR ceiling personalization to make that true, or (b) drop "Sweet Spot" from brand/roadmap language since the architecture is deliberately divergence-only now and there's nothing to surface.
