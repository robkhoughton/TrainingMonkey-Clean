-- ACWR Migration Monitoring and Logging System Database Schema
...
CREATE TABLE IF NOT EXISTS acwr_migration_logs (
    ...
    CONSTRAINT fk_migration_log_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL,
    
    -- Check constraints
    CONSTRAINT chk_migration_log_level CHECK (level IN ('DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL')),
    
    -- Indexes
    INDEX idx_migration_log_migration ON acwr_migration_logs(migration_id),
    INDEX idx_migration_log_user ON acwr_migration_logs(user_id),
    INDEX idx_migration_log_timestamp ON acwr_migration_logs(timestamp),
    INDEX idx_migration_log_level ON acwr_migration_logs(level),
    INDEX idx_migration_log_source ON acwr_migration_logs(source)
);
...
CREATE TABLE IF NOT EXISTS acwr_migration_alerts (
    ...
    -- Foreign key constraints
    CONSTRAINT fk_migration_alert_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL,
    CONSTRAINT fk_migration_alert_ack_by FOREIGN KEY (acknowledged_by) REFERENCES users(user_id) ON DELETE SET NULL,
    
    -- Check constraints
    CONSTRAINT chk_migration_alert_severity CHECK (severity IN ('low', 'medium', 'high', 'critical')),
    
    -- Indexes
    INDEX idx_migration_alert_migration ON acwr_migration_alerts(migration_id),
    INDEX idx_migration_alert_user ON acwr_migration_alerts(user_id),
    INDEX idx_migration_alert_timestamp ON acwr_migration_alerts(timestamp),
    INDEX idx_migration_alert_severity ON acwr_migration_alerts(severity),
    INDEX idx_migration_alert_type ON acwr_migration_alerts(alert_type),
    INDEX idx_migration_alert_acknowledged ON acwr_migration_alerts(acknowledged),
    INDEX idx_migration_alert_resolved ON acwr_migration_alerts(resolved)
);
...
CREATE TABLE IF NOT EXISTS acwr_migration_events (
    ...
    -- Foreign key constraints
    CONSTRAINT fk_migration_event_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE SET NULL,
    ...
    -- Indexes
    INDEX idx_migration_event_migration ON acwr_migration_events(migration_id),
    INDEX idx_migration_event_user ON acwr_migration_events(user_id),
    INDEX idx_migration_event_timestamp ON acwr_migration_events(timestamp),
    INDEX idx_migration_event_type ON acwr_migration_events(event_type),
    INDEX idx_migration_event_source ON acwr_migration_events(source)
);
...
CREATE TABLE IF NOT EXISTS acwr_migration_health_metrics (
    ...
    -- Indexes
    INDEX idx_migration_health_migration ON acwr_migration_health_metrics(migration_id),
    INDEX idx_migration_health_timestamp ON acwr_migration_health_metrics(timestamp),
    INDEX idx_migration_health_status ON acwr_migration_health_metrics(status),
    INDEX idx_migration_health_performance ON acwr_migration_health_metrics(performance_score)
);
...
CREATE TABLE IF NOT EXISTS acwr_migration_monitoring_config (
    ...
    -- Indexes
    INDEX idx_migration_monitoring_config_name ON acwr_migration_monitoring_config(config_name),
    INDEX idx_migration_monitoring_config_active ON acwr_migration_monitoring_config(is_active)
);
...
CREATE TABLE IF NOT EXISTS acwr_migration_notification_preferences (
    ...
    -- Foreign key constraints
    CONSTRAINT fk_migration_notification_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    ...
    -- Indexes
    INDEX idx_migration_notification_user ON acwr_migration_notification_preferences(user_id),
    INDEX idx_migration_notification_type ON acwr_migration_notification_preferences(notification_type),
    INDEX idx_migration_notification_enabled ON acwr_migration_notification_preferences(enabled)
);
...
GROUP BY ml.migration_id, ml.user_id, u.username
...
GROUP BY ma.migration_id, ma.user_id, u.username
...
CREATE OR REPLACE FUNCTION trigger_migration_log_monitoring()
RETURNS TRIGGER AS $$
BEGIN
    -- This trigger would automatically monitor log entries
    -- and create alerts for critical conditions
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        IF NEW.level = 'CRITICAL' THEN
            INSERT INTO acwr_migration_alerts (
                ...
                'alert_' || NEW.migration_id || '_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT,
                NEW.migration_id, NEW.user_id, CURRENT_TIMESTAMP, 'critical', 'critical_error',
                'Critical Error Detected', NEW.message, NEW.details
            );
        END IF;
        
        -- Create alert for high error rate
        IF NEW.level = 'ERROR' THEN
            -- Check if error rate is high (simplified check)
            IF (SELECT COUNT(*) FROM acwr_migration_logs 
                WHERE migration_id = NEW.migration_id 
                AND level = 'ERROR' 
                AND timestamp > CURRENT_TIMESTAMP - INTERVAL '1 hour') > 10 THEN
                
                INSERT INTO acwr_migration_alerts (
                    alert_id, migration_id, user_id, timestamp, severity, alert_type, title, message, details
                ) VALUES (
                    'alert_' || NEW.migration_id || '_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT,
                    NEW.migration_id, NEW.user_id, CURRENT_TIMESTAMP, 'high', 'high_error_rate',
                    'High Error Rate Detected', 'Multiple errors detected in the last hour', 
                    json_build_object('error_count', 10, 'time_window', '1 hour')
                );
            END IF;
        END IF;
    END IF;
...
-- Create trigger on migration logs table
-- DROP TRIGGER IF EXISTS trigger_migration_log_monitoring ON acwr_migration_logs;
-- CREATE TRIGGER trigger_migration_log_monitoring
--     AFTER INSERT ON acwr_migration_logs
--     FOR EACH ROW
--     EXECUTE FUNCTION trigger_migration_log_monitoring();
