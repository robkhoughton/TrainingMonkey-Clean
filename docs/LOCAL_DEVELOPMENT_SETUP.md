...
Before starting the Flask app, always check if port 5000 is already in use:
...
Open your browser and go to: `http://localhost:5000/landing`

## Common Issues & Solutions

### Issue: 404 Error on /landing
**Cause**: Port 5000 occupied by Docker or other service
**Solution**: 
1. Kill the conflicting process
2. Restart Flask app

### Issue: Debug Script Works But Browser Doesn't
**Cause**: Different service running on port 5000
**Solution**: Always check `netstat -ano | findstr :5000` first
...
3. **Test Changes**:
   - Browser: `http://localhost:5000/landing`
...
1. **Always check port conflicts first** - Docker often occupies port 5000
...
|-------|-------|----------|
| 404 on /landing | `netstat -ano \| findstr :5000` | Kill conflicting process |
