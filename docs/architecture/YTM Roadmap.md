...
- Athlete-model calibration pipeline is starved: `deviation_reason` is NULL on all 256 autopsies, so `divergence_injury_threshold` can never calibrate (`threshold_n=0`, while `typical_divergence_low` has n=60). Two writers exist — `classify_deviation()` (returns early on alignment ≥7 and several other gates) and `respond_alignment_query()` (only fires when an alignment query is answered, and they sit `pending` indefinitely). Application is now gated + conservative-floored (2026-08-23), so the safety floor is honest, but no threshold will ever *become* calibrated until this is fixed. Open sub-question: whether same-day divergence is the right readout for a breakdown threshold at all, or whether it should read a preceding window — do NOT decide this on single injury events.
