...
| 2026-06-01 | Anthropic docs page unavailable (HTTP 403) — no model ID changes made per rubric policy. Current IDs unchanged: `claude-sonnet-4-6` (default/Sonnet), `claude-haiku-4-5-20251001` (Haiku), `claude-opus-4-6` (Opus), `claude-haiku-4-5-20251001` (chat). `app/config.json` still absent; `llm_recommendations_module.py` constants remain live source of truth. Verify and update against docs next cycle. | Scheduled agent |
| 2026-09-01 | Anthropic docs page unavailable (egress blocked by network proxy) — no model ID changes made per rubric policy. Current IDs unchanged: `claude-sonnet-4-6` (default/Sonnet), `claude-haiku-4-5-20251001` (Haiku), `claude-opus-4-6` (Opus), `claude-haiku-4-5-20251001` (chat). `app/config.json` still absent; `llm_recommendations_module.py` constants remain live source of truth. Verify and update against docs next cycle. | Scheduled agent |
