...
Specification Clarity does not appear in daily Rx prose at all going forward — it's a precondition resolved upstream by the season-goal gate (05) before generation ever starts, so there's nothing left to report about it per-day once that gate exists.
...
**Blocked by:** 02, 05 (Data Reliability must exist to be cited; the season-goal gate must exist before it's safe to drop Specification Clarity from daily narration — otherwise a user with an unclear target loses the only place that was ever mentioned to them)
